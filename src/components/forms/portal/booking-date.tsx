        ...
        {availableSlots?.map((slot, key) => (
          <Label htmlFor={`slot-${key}`} key={key}>
            <Card
              onClick={() => onSlot(slot)}
              className={cn(
                currentSlot === slot ? 'bg-emerald-300' : 'bg-green-300',
                'px-10 py-4',
                bookings &&
                  bookings.some(
                    (booking) =>
                  ...
                  ? 'bg-gray-300'
                  : 'cursor-pointer border-green-300 hover:bg-emerald-300 transition duration-150 ease-in-out'
              )}
            >
              <Input
                {...(bookings &&
                bookings.some(
                  (booking) =>
                    booking.date.getTime() === date?.getTime() && booking.slot === slot
                )
                  ? {
                      disabled: true,
                    }
